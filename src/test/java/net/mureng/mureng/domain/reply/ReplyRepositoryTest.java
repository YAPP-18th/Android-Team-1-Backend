...
public class ReplyRepositoryTest {
    ...
    public void 멤버_답변_목록_조회(){
        List<Reply> replyList = replyRepository.findAllByMemberIdMemberId(MEMBER_ID);

        assertEquals(2, replyList.size());

        assertEquals(MEMBER_ID, replyList.get(0).getMemberId().getMemberId());
        assertEquals("yellow", replyList.get(0).getContent());
        assertEquals("yellow image", replyList.get(0).getImage());

        assertEquals(MEMBER_ID, replyList.get(1).getMemberId().getMemberId());
    ...
    public void 질문_답변_목록_조회(){
        List<Reply> replyList = replyRepository.findAllByQuestionIdQuestionId(QUESTION_ID);

        assertEquals(2, replyList.size());

        assertEquals(QUESTION_ID, replyList.get(0).getQuestionId().getQuestionId());
        assertEquals("yellow", replyList.get(0).getContent());
        assertEquals("yellow image", replyList.get(0).getImage());

        assertEquals(QUESTION_ID, replyList.get(1).getQuestionId().getQuestionId());
